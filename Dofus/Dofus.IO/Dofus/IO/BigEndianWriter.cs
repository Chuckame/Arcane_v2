...
namespace Dofus.IO
{
    internal class BigEndianWriter : IDataWriter, IDisposable
    {
        #region Properties
        ...
        internal BigEndianWriter(Stream stream)
        {
            m_writer = new BinaryWriter(stream, Encoding.UTF8);
        }

        internal BigEndianWriter(byte[] buffer)
        {
        ...
        {
            return new BigEndianWriter(Data);
        }
    }
