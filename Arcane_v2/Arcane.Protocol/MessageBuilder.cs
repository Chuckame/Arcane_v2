...
namespace Arcane.Protocol.Messages
{
    public class MessageBuilder : IMessageFactory<AbstractMessage>
    {
        ...
        private static short subComputeStaticHeader(uint id, byte typeLen)
        {
        ...
        public bool TryBuildMessages(byte[] raw, out ICollection<AbstractMessage> builtMessages)
        {
            if (raw.Length < 2)
            {
                builtMessages = null;
            ...
            using (var reader = DofusIOUtils.CreateFastBigEndianReader(raw))
            {
                while (reader.BytesAvailable >= 2)
                {
                    var header = reader.ReadUShort();
                    var messageId = (uint)(header >> 2);
                    var typeLen = header & 3;
                    if (reader.BytesAvailable >= typeLen)
                    {
                        var length = readMessageLength(typeLen, reader);
                        rawMessages.Add(new RawMessage { Data = reader.ReadBytes(length), MessageId = messageId });
                    }
                }
            ...
            builtMessages = new List<AbstractMessage>();
            foreach (var rm in rawMessages)
            {

                var msg = CreateMessageInstance(rm.MessageId);
                using (var reader = DofusIOUtils.CreateBigEndianReader(rm.Data))
                {
