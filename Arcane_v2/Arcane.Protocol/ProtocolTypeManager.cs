...
namespace Arcane.Protocol.Types
{
    public static class ProtocolTypeManager
    {
        ...
        private static readonly Dictionary<short, Func<AbstractType>> m_typesConstructors = new Dictionary<short, Func<AbstractType>>();

        static ProtocolTypeManager()
        {
            var asm = Assembly.GetAssembly(typeof(ProtocolTypeManager));

            foreach (Type type in asm.GetTypes().Where(t => t.IsSubclassOf(typeof(AbstractType))))
            {
                ...
                if (field != null)
                {
                    ...
                    m_typesConstructors.Add(id, () => (AbstractType)ctor.Invoke(new object[0]));
                }
            }
        }
        ...
        /// <returns></returns>
        public static T GetInstance<T>(short id) where T : AbstractType
        {
